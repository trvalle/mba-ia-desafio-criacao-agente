...
class Database:
    ...
    def initialize(self) -> None:
        with self.transaction(immediate=True) as conn:
            conn.executescript(
                ...
                CREATE UNIQUE INDEX IF NOT EXISTS uq_active_area_date
                    ON reservations(area, date) WHERE status = 'active';
            ...
            if conn.execute("SELECT 1 FROM metadata WHERE key='reservation_counter'").fetchone() is None:
                conn.execute("INSERT INTO metadata(key, value) VALUES('reservation_counter', '1000')")

    def create_session(self, apartment: str) -> str:
        session_id = str(uuid.uuid4())
        with self.transaction(immediate=True) as conn:
            conn.execute(
                "INSERT INTO sessions(id, apartment, created_at) VALUES (?, ?, ?)",
    ...
    def add_event(self, session_id: str, event_type: str, author: str, content: Any) -> None:
        with self.transaction(immediate=True) as conn:
            ...
            conn.execute(
                "INSERT INTO session_events(session_id, sequence, event_type, author, content, created_at) VALUES (?, ?, ?, ?, ?, ?)",
    ...
    def events(self, session_id: str) -> list[dict[str, Any]]:
        ...
        try:
            rows = conn.execute(
                "SELECT sequence, event_type, author, content, created_at FROM session_events WHERE session_id=? ORDER BY sequence",
                (session_id,),
            ).fetchall()
            return [
                {
    ...
    def pending_confirmations(self, session_id: str) -> list[dict[str, Any]]:
        ...
        try:
            rows = conn.execute(
                "SELECT id, kind, details FROM confirmations WHERE session_id=? AND status='pending' ORDER BY created_at, id",
                (session_id,),
            ).fetchall()
            return [
...
def restore_initial_data(path: Path | str = DATABASE_PATH, clear_sessions: bool = True) -> None:
    ...
    with db.transaction(immediate=True) as conn:
        ...
        conn.execute("UPDATE metadata SET value='1000' WHERE key='reservation_counter'")
